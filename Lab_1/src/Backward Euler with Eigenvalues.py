...
def backward_euler(fun, x0: np.ndarray, t_f: float, h: float) -> tuple:
    ...
    for k in range(len(t) - 1):
        ...
        e = 1  # Initial error value
        while e > 1e-8:  # Iterate until convergence
            x_n = x_hist[:, k] + h * fun(x_hist[:, k + 1])
            e = np.linalg.norm(x_n - x_hist[:, k + 1])  # Compute error
            x_hist[:, k + 1] = x_n  # Update the state
...
# Run the backward Euler integration
x_hist3, t_hist3 = backward_euler(mass_spring_system, x0, t_f, h)
...
# Stability analysis
h_values = np.linspace(0.001, 0.1, 100)  # Avoid zero for stability analysis
...
plt.title('Eigenvalue Norm for Mass-Spring System Stability')
plt.grid()
plt.tight_layout()
plt.show()
